...
let lookup_primitive loc ~poly_mode ~poly_sort pos p =
  ...
  let prim = match p.prim_name with
    ...
    | "%array_safe_get_indexed_by_int64#" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, gen_array_kind,
          Punboxed_int_index Unboxed_int64, Mutable)), 2)
    ...
    | "%array_safe_get_indexed_by_int32#" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, gen_array_kind,
          Punboxed_int_index Unboxed_int32, Mutable)), 2)
    ...
    | "%array_safe_get_indexed_by_nativeint#" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, gen_array_kind,
          Punboxed_int_index Unboxed_nativeint, Mutable)), 2)
    ...
    | "%obj_set_field" ->
      Primitive
        ((Parraysetu (Pgenarray_set (get_first_arg_mode ()),
          gen_array_kind, Ptagged_int_index)), 3)
    | "%floatarray_length" ->
    ...
    | "%floatarray_safe_get" ->
      Primitive ((Parrayrefs (Pfloatarray_ref mode, Pfloatarray,
        Ptagged_int_index, Mutable)), 2)
    | "%floatarray_safe_set" ->
      Primitive (Parraysets (Pfloatarray_set, Pfloatarray, Ptagged_int_index), 3)
...
let specialize_primitive env loc ty ~has_constant_constructor prim =
  ...
  | Primitive (Parrayrefu (rt, array_kind, index_kind, mut), arity), p1 :: _ ->
    begin
      let loc = to_location loc in
      let array_type = array_type_kind ~elt_sort:None env loc p1 in
      let array_ref_type = glb_array_ref_type loc rt array_type in
  ...
  | Primitive (Parraysetu (st, array_kind, index_kind), arity), p1 :: _ -> begin
      let loc = to_location loc in
      let array_type = array_type_kind ~elt_sort:None env loc p1 in
      let array_set_type = glb_array_set_type loc st array_type in
      let array_kind' = glb_array_type loc array_kind array_type in
  ...
  | Primitive (Parrayrefs (rt, array_kind, index_kind, mut), arity), p1 :: _ ->
    begin
      let loc = to_location loc in
      let array_type = array_type_kind ~elt_sort:None env loc p1 in
      let array_ref_type = glb_array_ref_type loc rt array_type in
  ...
  | Primitive (Parraysets (st, array_kind, index_kind), arity), p1 :: _ -> begin
      let loc = to_location loc in
      let array_type = array_type_kind ~elt_sort:None env loc p1 in
      let array_set_type = glb_array_set_type loc st array_type in
      let array_kind' = glb_array_type loc array_kind array_type in
