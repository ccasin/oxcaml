  ...
  let additional_action : additional_action =
    ...
    | Prepare_for_saving ->
        let prepare_jkind (type l r) loc (jkind : (l * r) jkind) =
          ...
          | Some const ->
            begin
              match Jkind_builtins_memo.find ~quality:jkind.quality const with
              | Some jkind -> jkind
              | None ->
                Jkind_jkind.of_const ~quality:jkind.quality const
...
let jkind' copy_scope s loc jkind =
  let jkind =
    match s.additional_action with
    | Prepare_for_saving { prepare_jkind } -> prepare_jkind loc jkind
