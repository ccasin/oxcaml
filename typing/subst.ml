...
let jkind s jkind =
  ...
  | Kconstr p ->
    ...
    | exception Not_found ->
      { jkind with base = Kconstr (jkind_path s p) }
    | Jkind_path _ ->
      { jkind with base = Kconstr (jkind_path s p) }
    | Jkind_const { base; mod_bounds; with_bounds = No_with_bounds } ->
      let const =
        { base;
          mod_bounds = Jkind_mod_bounds.meet mod_bounds jkind.mod_bounds;
          with_bounds = jkind.with_bounds }
      in
      Jkind_base_and_axes.map_layout Jkind_types.Layout.of_const const
    end
  | Layout _ ->
    (* CR layouts: No paths in here, but for lpoly we'll probably need to
       descend and update levels on sort vars as for type vars? *)
    jkind
...
let jkind_desc copy_scope s loc jk =
  let jk =
    match s.additional_action with
    | Prepare_for_saving { prepare_jkind; _ } -> prepare_jkind loc jk
    | Duplicate_variables | No_action -> jk
  in
  let jkind_desc = jkind s jk.jkind in
  let jk = { jk with jkind = jkind_desc } in
  Jkind_jkind.map_type_expr (typexp copy_scope s loc) jk

let jkind_const_desc s (jkind : jkind_const_desc_lr) =
  let jkind =
    match jkind.base with
    | Kconstr p ->
...
let rec type_declaration' copy_scope s decl =
  ...
  { type_params = List.map (typexp copy_scope s decl.type_loc) decl.type_params;
    ...
    type_jkind = jkind_desc copy_scope s decl.type_loc decl.type_jkind;
