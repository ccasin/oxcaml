...
(* This module exists here to resolve a dependency cycle ([Subst] must not
   ...
   modules are re-exported as [Jkind.With_bounds]. *)
module Jkind_with_bounds : sig
  ...
  val relevant_axes_of_modality :
    ...
    modality:Mode.Modality.Value.Const.t -> Jkind_axis.Axis_set.t
...
(* This module exists here to resolve a dependency cycle ([Subst] must not
   depend on [Jkind]).  The portions intended for use outside of those two
   modules are re-exported as [Jkind], and documented in [jkind.mli] *)
module Jkind_jkind : sig
  ...
  module Builtin : sig
    ...
    val any_non_null : why:Jkind_intf.History.any_creation_reason -> 'd jkind
    val void :
    ...
    val product :
      ...
      (type_expr * Mode.Modality.Value.Const.t) list ->
