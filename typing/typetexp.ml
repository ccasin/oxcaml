...
module TyVarEnv : sig
  ...
  val make_poly_univars_jkinds :
    context:(string -> Jkind.History.annotation_context_lr) -> Env.t ->
  ...
  *)
  ...
  let mk_poly_univars_tuple_with_jkind ~context env var jkind_annot stage =
    let name = var.txt in
    let original_jkind =
  ...
  let make_poly_univars_jkinds ~context env vars_jkinds =
    let mk_trip = function
        | (v, None, s) -> mk_poly_univars_tuple_without_jkind (v, s)
        | (v, Some l, s) -> mk_poly_univars_tuple_with_jkind ~context env v l s
    in
...
let transl_type_param env path jkind_default styp =
  ...
  let transl_jkind_and_annot_opt jkind_annot name =
    ...
    | Some jkind_annot ->
      let jkind =
        Jkind.of_annotation env ~context:(Type_parameter (path, name))
          jkind_annot
      in
