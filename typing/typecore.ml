...
and type_pat_aux
  : type k . type_pat_state -> k pattern_category -> no_existentials:_ ->
         ...
         penv:Pattern_env.t -> zero_alloc:_ -> _ -> _ -> _ -> k general_pattern
  = fun tps category ~no_existentials ~alloc_mode ~mutable_flag ~penv
        ~zero_alloc sp expected_ty sort ->
  ...
  let type_record_pat (type rep) (record_form : rep record_form) lid_sp_list
      ...
      let type_label_pat sorts (label_lid, (label : rep gen_label_description),
        ...
        (label_lid, label,
         type_pat tps Value ~alloc_mode sarg ty_arg ty_sort ~zero_alloc)
      in
      let make_record_pat
            sorts (rep : rep)
  ...
  let rec verify_zero_alloc desc zero_alloc =
    ...
    | Ppat_record _ | Ppat_record_unboxed_product _ | Ppat_effect _ ->
  ...
  | Ppat_array (mut, spl) ->
      ...
      let pl =
        List.map
          (fun p ->
             type_pat ~alloc_mode tps Value p ty_elt arg_sort ~zero_alloc)
          (* XXX surely should be default *)
          spl
      in
      rvp {
...
let type_pattern
    category ~lev ~alloc_mode ~zero_alloc env spat expected_ty ?cont sort
    allow_modules
  =
  ...
  | Pexp_match(sarg, caselist) ->
      ...
      let val_cases, partial =
        type_cases Computation env arg_pat_mode expected_mode arg.exp_type
          sort ty_expected_explained ~check_if_total:true ~zero_alloc:None
          loc val_caselist
      in
      let eff_cases =
        match eff_caselist with
  ...
  | Pexp_try(sbody, caselist) ->
      ...
      let exn_cases, _ =
        type_cases Value env arg_mode expected_mode
          ...
          ty_expected_explained
          ~check_if_total:false ~zero_alloc:None loc exn_caselist
      in
      let eff_cases =
        match eff_caselist with
        | [] -> []
...
and map_half_typed_cases
  : type k ret case_data.
    ...
    -> ret list * partial
  = fun ?additional_checks_for_split_cases ?conts ~zero_alloc
          ...
          let htc =
            with_local_level_generalize_structure_if_principal begin fun () ->
              ...
              let (pat, ext_env, force, pvs, mvs) =
                type_pattern ?cont category ~lev ~alloc_mode:pat_mode
                  ~zero_alloc env pattern ty_arg sort_arg allow_modules
              in
      ...
      with_local_level begin fun () ->
        ...
        let cases, _ =
          type_cases category new_env (simple_pat_mode Value.legacy) rhs_mode
            ty_arg sort_eff ty_res_explained ~conts ~check_if_total:false
            ~zero_alloc:None loc caselist
        in
          cases
        end
