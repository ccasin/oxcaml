...
type error =
  ...
  | Jkind_empty_record
  | Non_value_in_sig of Env.t * Jkind.Violation.t * string * type_expr
...
let transl_value_decl env loc ~modalities valdecl =
  ...
  | Error err ->
    raise(Error(cty.ctyp_loc,
                Non_value_in_sig(env, err,valdecl.pval_name.txt,cty.ctyp_type)))
  end;
  let ty = cty.ctyp_type in
  let v =
...
let report_error ppf = function
  ...
  | Non_value_in_sig (env, err, val_name, ty) ->
