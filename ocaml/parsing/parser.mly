...
strict_function_or_labeled_tuple_type:
  ...
  | mktyp(
    ...
    { $1 }
  (* These next two cases are for labled tuples - see comment on [tuple_type]
     below.

     The first case is present just to resolve a shift reduce conflict
     in a module type [S with t := x:t1 * t2 -> ...] which might be the
     beginning of
       [S with t := x:t1 * t2 -> S']    or    [S with t := x:t1 * t2 -> t3]
     This production is just the previous one, but with [arg_label] replaced
