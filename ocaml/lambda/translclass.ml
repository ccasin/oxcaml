  ...
  | Tcl_fun (_, pat, vals, cl, partial) ->
      ...
      (inh_init,
       let build params rem =
         ...
         in
         let body =
           Matching.for_function ~scopes ~arg_layout
             ~arg_sort:Sort.sort_class_arg ~result_layout:layout_obj pat.pat_loc
             None (Lvar param) [pat, rem] partial
         in
         Lambda.lfunction
                   ~kind:(Curried {nlocal=0}) ~params:((param, arg_layout)::params)
                   ~return:layout_obj
                   ~attr:default_function_attribute
      ...
      let build params rem =
        ...
        let arg_layout =
          Typeopt.layout pat.pat_env pat.pat_loc Sort.sort_class_arg
            pat.pat_type
        in
        let result_layout = layout_class in
        let body =
          Matching.for_function ~scopes ~arg_layout
            ~arg_sort:Sort.sort_class_arg ~result_layout pat.pat_loc
            None (Lvar param) [pat, rem] partial
        in
        Lambda.lfunction
                  ~kind:(Curried {nlocal=0}) ~params:((param, arg_layout)::params)
