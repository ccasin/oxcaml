...
let comp_primitive p args =
  ...
  (* For the bytecode compiler, projecting a float# _should_ box it.  So, we can
     use the existing float instructions for the ufloat primitives. *)
...
let rec comp_expr env exp sz cont =
  ...
  | Lprim ((Pmakefloatblock _ | Pmakeufloatblock _), args, loc) ->
      (* In bytecode, unboxed floats are boxed, so we can treat these two
         primitives the same. *)
      let cont = add_pseudo_event loc !compunit_name cont in
