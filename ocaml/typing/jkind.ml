...
module Const = struct
  ...
  let rec get_legacy_layout
    ...
    | Base Value, _, Maybe_null -> Value_or_null
    | Base Value, Internal, Non_null -> Value
    | Base Value, External64, Non_null -> Immediate64
    | Base Value, External, Non_null -> Immediate
    | Base Void, _, _ -> Void
    | Base Float64, _, _ -> Float64
    | Base Float32, _, _ -> Float32
    | Base Word, _, _ -> Word
    | Base Bits32, _, _ -> Bits32
    | Base Bits64, _, _ -> Bits64
    | Product layouts, _, _ ->
      Product
