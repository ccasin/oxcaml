...
(* XXX this example below crashes the native code compiler and must be fixed
   before merging this.  Leaving it like this for now while I discuss with the
   middle-end folks. *)
let _ = let rec _x = #(3, 10) and _y = 42 in 42
[%%expect{|
Line 1, characters 16-17:
1 | let _ = let rec x = #(3, 10) and y = 42 in 42
                    ^
