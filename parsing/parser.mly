...
(* This grammar is parameterized by itself so that we can sometimes include with
   ...
   but in practice they create problems for parsing around with constraints. *)
jkind_desc_gen(self):
    jkind_annotation_gen(self) MOD
    mkrhs(LIDENT)+ { (* LIDENTs here are for modes *)
      let modes =
        List.map
...
(* Base jkind grammar without WITH - used in with_constraints *)
