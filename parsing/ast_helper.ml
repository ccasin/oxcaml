  ...
  let varify_constructors var_names t =
    ...
    and loop_jkind jkind =
      let pjka_desc =
        ...
        | Pjka_default as x -> x
        | Pjka_abbreviation _ as x -> x
        | Pjka_mod (jkind, modes) -> Pjka_mod (loop_jkind jkind, modes)
        | Pjka_with (jkind, typ, modalities) ->
          Pjka_with (loop_jkind jkind, loop typ, modalities)
        | Pjka_kind_of typ -> Pjka_kind_of (loop typ)
        | Pjka_product jkinds -> Pjka_product (List.map loop_jkind jkinds)
      in
